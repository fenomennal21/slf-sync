...
{block name='footer-script'}
	<script type="text/javascript">
		$(function(){
			...
			var refreshTable = function(kodeProdi, angkatan) {
				var url = '{site_url('sync/mahasiswa_data')}/'+kodeProdi+'/'+angkatan;
				
				if (kodeProdi !== null && angkatan !== null)
				{
			...
			$('select[name=kode_prodi]').on('change', function(){
				refreshTable($('select[name=kode_prodi]').val(), $('select[name=angkatan]').val());
			});
